...
from ultralytics.nn.modules.block import PmbConv, C2f_Pmb
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        elif m in (Detect, Detect_DyHead, Segment, Pose):
        ...
        elif m in {SpatialAttention, CBAM, PmbConv}:
            c2 = ch[f]
            args = [c2, *args]
            # print(args)
...
def yaml_model_load(path):
    ...
    unified_path = re.sub(r'(\d+)([nslmx])(.+)?$', r'\1\3', str(path))  # i.e. yolov8x.yaml -> train1.yaml
